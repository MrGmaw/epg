...
CNN en Español, NTN24 y los nueve canales añadidos desde mi.tv se procesan
interpretando las horas del endpoint asíncrono como UTC y convirtiéndolas a
`America/Guayaquil`.

Los nueve canales añadidos directamente a `latam.xml` son:
...
El workflow intenta obtener logos para estos 11 canales:
...
`ec.xml` recibe esta sustitución para CNN en Español y NTN24. `latam.xml` la
aplica a los once canales de mi.tv. Si todavía no existe logo para un canal,
la guía se publica sin inventar un `<icon>` roto.
...
- `logos/manifest.json` con los 11 objetivos de mi.tv;
