...
- Añade `OromarTV.ec` desde AmericaTVGuide Ecuador.
- Lleva ambos horarios a `America/Guayaquil` sin offset manual.
- CBS usa una cadena de respaldo TVPassport HD → TVPassport SD → EPGShare US1 → `.cache/previous-latam.xml`; Oromar conserva fallback al XML previo.
