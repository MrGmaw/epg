#!/usr/bin/env python3
"""Construye ``latam.xml`` con los 20 canales seleccionados.
...
ocho canales de mi.tv y la parrilla oficial de Ecuador TV cuando está
 disponible.
"""
...
def validate_latam_tree(
    ...
    if len(channel_ids) != 20 or len(set(channel_ids)) != 20:
        raise RuntimeError("latam.xml debe contener exactamente 20 canales únicos.")
...
def self_test() -> None:
    ...
    assert len(LATAM_CHANNEL_IDS) == 20
    assert len(set(LATAM_CHANNEL_IDS)) == 20
    assert LATAM_CHANNEL_IDS[-1] == ECUADOR_TV_ID
    print(
        "Prueba latam correcta: 20 IDs únicos y parser de Ecuador TV validado."
    )
