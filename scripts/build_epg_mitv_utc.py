...
La respuesta asíncrona usada para CNN en Español y NTN24 publica sus horas
en UTC. El generador base se conserva intacto y esta capa sustituye únicamente
``parse_mitv_page`` para convertir UTC -> America/Guayaquil antes de crear los
elementos XMLTV.
"""
...
def parse_mitv_page_utc(
    ...
    """Lee mi.tv en UTC y devuelve emisiones convertidas a Ecuador.

    El orden y los selectores permanecen iguales a los del generador base.
    La detección de cambio de día se hace todavía en la zona de origen (UTC),
    y solo después cada inicio se convierte a ``America/Guayaquil``.
    """
    ...
    for item in items:
        ...
        if (
            previous_source_start is not None
...
def main() -> int:
    # scrape_mitv_range() consulta esta función global al ejecutarse, por lo que
    # una sola sustitución corrige simultáneamente CNN en Español y NTN24.
