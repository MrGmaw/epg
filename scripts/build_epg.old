...
- ReporTV Finder, renderizado con Chromium cuando la grilla requiere JavaScript,
  para la programación diaria de Oromar TV.
...
def fetch_rendered_html(url: str, *, timeout: int = 120) -> str:
    """Renderiza una página con Chromium y devuelve el DOM final.

    ReporTV entrega inicialmente una carcasa HTML y carga la grilla mediante
    JavaScript. `requests` no ejecuta ese código, por lo que se utiliza
    Playwright únicamente como mecanismo de respaldo.
    """
    ...
    try:
        ...
        raise RuntimeError(
            "ReporTV requiere JavaScript, pero Playwright no está instalado."
        ) from exc

    log(f"Renderizando con Chromium: {url}")
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            try:
                ...
                # Algunas páginas mantienen conexiones abiertas. El contenido
                # puede estar listo aunque no se alcance networkidle.
                log("ReporTV: networkidle agotó el tiempo; se continúa.")

            try:
                page.wait_for_function(
                    "document.body && /OROMAR/i.test(document.body.innerText)",
                    timeout=45_000,
                )
            except PlaywrightTimeoutError:
                log(
                    "ReporTV: no apareció OROMAR durante la espera; "
                    "se analizará el DOM disponible."
                )

            # Permite que terminen de insertarse las tarjetas de programación.
    ...
    if not rendered.strip():
        raise RuntimeError("ReporTV devolvió un DOM renderizado vacío.")

    if "oromar" not in normalized_key(visible_text):
        raise RuntimeError(
            "ReporTV fue renderizado, pero el texto visible no contiene OROMAR."
        )
...
def extract_oromar_from_html(page: str) -> list[ScheduleItem]:
    soup = BeautifulSoup(page, "lxml")
    ...
    for candidate in candidates:
        ...
        while node is not None:
            lines = lines_from_tag(node)
            time_count = sum(
                1 for line in lines if REPORTV_TIME_RE.fullmatch(line)
            )
            oromar_count = sum(
                1 for line in lines if "oromar" in normalized_key(line)
            )
    ...
    if best is not None:
        return parse_reportv_schedule(lines_from_tag(best[1]))

    # Respaldo para variantes del DOM en las que el nombre del canal y sus
    # tarjetas no comparten un contenedor pequeño.
    lines = lines_from_tag(soup)
    positions = [
        idx
        for idx, line in enumerate(lines)
        if "oromar" in normalized_key(line)
    ]

    for position in positions:
        # Se prueban ventanas progresivas: una corta evita mezclar el canal
        # siguiente; las mayores toleran componentes con texto auxiliar.
        for window_size in (80, 140, 250):
            window = lines[
                max(0, position - 5) : position + window_size
            ]
            try:
                return parse_reportv_schedule(window)
            except RuntimeError:
                continue

    raise RuntimeError(
        "Oromar/ReporTV: no se encontró un bloque de programación utilizable."
    )


def scrape_oromar() -> list[ScheduleItem]:
    static_page = fetch_text(REPORTV_URL)

    try:
        items = extract_oromar_from_html(static_page)
        log(
            f"Oromar/ReporTV: {len(items)} emisiones encontradas "
            "en el HTML inicial."
        )
        return items
    except RuntimeError as static_error:
        log(
            "Oromar/ReporTV: la respuesta inicial no contiene la grilla "
            f"({static_error}). Se activará Chromium."
        )

    rendered_page = fetch_rendered_html(REPORTV_URL)
    items = extract_oromar_from_html(rendered_page)
    log(
        f"Oromar/ReporTV: {len(items)} emisiones encontradas después "
