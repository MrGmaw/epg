...
Si GatoTV no entrega ninguna parrilla utilizable, se intenta primero reutilizar
emisiones de TVEStarHD.es de la última latam.xml que coincidan con fechas
absolutas de la ventana vigente. Si esa intersección no es suficiente, se aplica
el mismo principio resiliente usado por TVC: la última semana válida de
``epg-data/latam.xml`` se agrupa por día de la semana y se proyecta únicamente
sobre el mismo weekday, conservando hora local de Guayaquil, duración, título y
descripción. No se desplazan horas ni se aplican correcciones manuales.
"""
...
def make_resilient_scraper(original: Callable, previous_latam_xml: Path) -> Callable:
    def resilient(config, start_date: date, days: int):
        ...
        try:
            ...
            latam.epg.warn(
                ...
                "se probará epg-data conservando fechas exactas y completando "
                "solo los días faltantes por el mismo día de la semana. "
                f"Detalle: {exc}"
            )
...
def _record_status(output_dir: Path) -> None:
    ...
    status["star_tve_cache_policy"] = (
        "gatotv-live -> exact-dates -> weekly-same-weekday-local-clock"
    )
